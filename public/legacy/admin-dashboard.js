...
function renderizarGraficas(deportes, resumen) {
    ...
    // Gr�fica por deporte (Barras horizontales)
...
function renderizarTablaDesgloseMensual(desglose) {
    const tbody = document.getElementById('tablaDesgloseMensual');
    if (!tbody) return;
    ...
    if (!desglose || desglose.length === 0) {
        tbody.innerHTML = `
            <tr>
                <td colspan="4" class="px-4 py-8 text-center text-text-muted">
                    Sin datos de historial mensual confirmado
                </td>
            </tr>
        `;
        return;
    ...
    tbody.innerHTML = desglose.map(m => {
        const mesLabel = MESES_ES[(m.mes || '').toLowerCase()] || m.mes;
        const total = parseFloat(m.total_recaudado || 0);
        const cantidad = parseInt(m.cantidad_pagos || 0);
        const deporte = m.deporte || '-';
        return `
            <tr class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors">
                <td class="px-4 py-3 font-semibold text-black dark:text-white">${mesLabel} ${m.anio || ''}</td>
                <td class="px-4 py-3 text-text-muted dark:text-gray-300">${deporte}</td>
                <td class="px-4 py-3 text-center font-mono">${cantidad}</td>
                <td class="px-4 py-3 text-right font-bold font-mono text-primary">S/ ${total.toFixed(2)}</td>
            </tr>
        `;
    }).join('');
}
...
function prepararDatosExportacion(stats) {
    ...
    return {
        ...
        matriculasPagadas: resumen.totalMatriculas / 20, // Asumiendo matr�cula de S/20
...
async function exportarDashboardExcel() {
    ...
    const resumenData = [
        ...
        ['Total Matr�culas Cobradas', Math.floor(estadisticasGlobales.matriculasPagadas)],
        ...
        ['Matr�culas', estadisticasGlobales.distribucion.matriculas.toFixed(2), `${((estadisticasGlobales.distribucion.matriculas / estadisticasGlobales.totalIngresos) * 100).toFixed(1)}%`],
    ...
    wsresumen['!merges'] = [
        ...
        { s: { r: 13, c: 0 }, e: { r: 13, c: 2 } } // DISTRIBUCI�N
    ];
    ...
    const deportesData = [
        ...
        [`Fecha: ${fecha}`],
        [''],
        ['Deporte', 'Matr�culas (S/)', 'Mensualidades (S/)', 'Total (S/)', 'Part. %']
    ];
    ...
    const alumnosData = [
        ...
        [`Fecha: ${fecha}`],
        [''],
        ['Pos.', 'DNI', 'Nombres y Apellidos', 'Deportes Inscritos', 'Matr�culas (S/)', 'Mensualidades (S/)', 'Total (S/)']
    ];
    ...
    // ==================== HOJA 4: AN�LISIS Y M�TRICAS ====================
    const analisisData = [
        ['AN�LISIS Y M�TRICAS ADICIONALES'],
        [`Fecha: ${fecha}`],
        [''],
        ['INDICADORES CLAVE'],
        [''],
        ...
        ['Ratio Matr�cula/Mensualidad', `${((estadisticasGlobales.distribucion.matriculas / estadisticasGlobales.distribucion.Mensualidades) * 100).toFixed(1)}%`, 'Proporci�n de ingresos por matr�culas vs Mensualidades'],
        ['Deportes Activos', estadisticasGlobales.deportes.length, 'Cantidad de deportes generando ingresos'],
        [''],
        [''],
        ['DEPORTE M�S RENTABLE'],
        ['']
    ];
    
    // Encontrar deporte m�s rentable
    ...
    analisisData.push(['Participaci�n:', `${((deporteMasRentable.total / estadisticasGlobales.totalIngresos) * 100).toFixed(1)}%`]);
